...
| 3.1 Options: the codeblocks(options) Starlight plugin, the… | not started | |
| 3.2 The range parser and the comment notation parser, with the comment… | not started | |
| 3.3 The client module loader, so that each page loads only the modules… | not started | |
| 3.4 Style settings and theme handling: the shared tokens, light and… | not started | |
| 3.5 Docs: the getting started page, the configuration reference page… | not started | |
| 4.1 Focus | not started | |
| 4.2 Line states, including custom states | not started | |
