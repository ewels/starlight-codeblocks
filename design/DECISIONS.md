...
- Decision: The sidebar groups the features by what authors use them for, in nine groups of two to four pages: Draw attention (focus, line states), Explain code (annotations, footnotes, inline callouts, side-by-side annotations), Connect prose and code (code mentions, scrollycoding), Show changes (word-level diff, token transitions), Shorten long code (hidden lines, expandable blocks), Make code easier to read (visible whitespace, colourised brackets, inline code highlighting), Add links (token links, API auto-linking, line permalinks), Adapt to the reader (code switcher, fill-in placeholders) and Copy and run (smart shell copy, open in playground, run in the browser). Comment notation is syntax that many features read, so it moves to "Start here". Page URLs do not change. The README, `llms.txt`, the accessibility page and the home page use the same groups in the same order.
- Reason: The user found the groups by where a feature works ("Inside the code block", "Across the page", "More") not useful. Scrollycoding goes with code mentions because both tie a part of the prose to lines, and token transitions go with word-level diff because both show how code changes.
- Alternatives: A "Walk through code" group for scrollycoding and token transitions, which left word-level diff alone.
