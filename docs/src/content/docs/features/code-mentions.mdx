...
The `starlight-links-validator` plugin reports `#mention:` links as broken, because no heading has that id. Exclude them in `astro.config.mjs`:

```js title="astro.config.mjs"
starlightLinksValidator({
  exclude: ({ link }) => link.startsWith('#mention:'),
}),
```
