...
|---|---|---|---|
| `playgrounds` | `false \| object` | on | Set to `false` to turn the feature off. `playground` attributes then have no effect. |
| `playgrounds.<name>` | `{ label, url }` or `{ label, post }` | none | A custom playground. A custom playground with a built-in name replaces the built-in one. |
...
export const python = `
\`\`\`py playground="pythontutor" apiLinks=false
...
A JavaScript example opens in StackBlitz. StackBlitz takes a project in a form post, so the button is part of a form:
